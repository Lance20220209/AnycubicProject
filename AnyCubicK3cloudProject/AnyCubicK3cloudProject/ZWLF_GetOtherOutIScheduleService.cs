...
namespace AnyCubicK3cloudProject
{
    ...
    [Description("获取胜途其他出库库定时任务插件")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_GetOtherOutIScheduleService : IScheduleService
    {
        /// <summary>
        /// 定时任务
        /// </summary>
        ...
        public void Run(Kingdee.BOS.Context ctx, Schedule schedule)
        {
            try
            {
                ...
                string sql = string.Format(@"/*dialect*/ select Id, app_key,AppSecret,date_type,end_date,page_no,page_size,
                                    ...
                                    from ZWLF_T_Configuration where method in('frdas.erp.otherout.get') and IsDisable=0");
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        //生成其他入库单
                        msg = zWLF_GetOtherOut.GetOtherOut(parameters, context);
                        if (msg.status)
                        {
