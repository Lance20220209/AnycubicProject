...
{
    [Description("处理失败Amazon的订单")]
    [Kingdee.BOS.Util.HotUpdate]
    public class ZWLF_GetFailSaleOrderScheduleService : IScheduleService
    {
        /// <summary>
        /// 定时任务
        /// </summary>
        ...
        public void Run(Kingdee.BOS.Context ctx, Schedule schedule)
        {
            try
            {
                ...
                sql += string.Format(@"/*dialect*/ select  top 100 id ,method  from ZWLF_T_SaleOut  where IsPush =10  and  site_type='Amazon' order  by send_date asc ");
                DataSet ds = DBServiceHelper.ExecuteDataSet(ctx, sql);
                DataTable dt = ds.Tables[0];
                DataTable dt_so = ds.Tables[1];
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        DateTime Btime = Convert.ToDateTime(dt.Rows[i]["start_date"].ToString()).AddMonths(-1);
                    ...
                    {
                        //循环订单
                        for (int i = 0; i < dt_so.Rows.Count; i++)
                        {
                            parameters.method = dt_so.Rows[i]["method"].ToString();
                            parameters.id = dt_so.Rows[i]["id"].ToString();
                            SaleTradeGet(parameters, ctx);
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                if (code == "200")
                {
                    ...
                    if (total_count > 0)
                    {
                        ...
                        //订单表
                        sql += string.Format(@"/*dialect*/ select FID,F_ZWLF_ID ,F_ZWLF_SITE_TRADE_ID from T_SAL_ORDER where FSALEORGID=165223 and F_ZWLF_ID!='' ;");
                        //币别
                        ...
                        //其他出库单
                        sql += string.Format(@"/*dialect*/ select  distinct FID,F_ZWLF_OrderNo ,F_ZWLF_ID from T_STK_MISDELIVERY where  F_ZWLF_OrderNo!=''");
                        ...
                        if (FStockOrgId == "165223")
                        {
                            ...
                            if (OrderRow.Length == 0)
                            {
                                ...
                                sql = string.Format(@"/*dialect*/ INSERT INTO ZWLF_T_OrderLog
                                                      ...
                                                      ,[F_ZWLF_TIME]
                                                      ,F_ZWLF_SOURCETYPE,F_ZWLF_IsthirdStock)
                                                    VALUES
                                ...
                                //如果是德瑞店铺海外仓发货的销售组织就是香港纵维
                                if (array[i]["site_user"].ToString() == "anycubic德瑞帕特专卖店" || array[i]["site_user"].ToString() == "szdrpt")
                                {
                                    FSaleOrgId = "165223";
                                }
                                //获取币别
                                ...
                                //判断是否售后
                                bool isSH = false;
                                if (array[i]["site_user"].ToString() == "001" && array[i]["site_name"].ToString() == "品牌店")
                                {
                                    isSH = true;
                                }
                                ...
                                string send_date = "";
                                //获取当月第一天
                                string MonthFirst = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1).ToString("yyyy-MM-dd");  //获取当月第一天
                                //如果发货时间是之前月份的则生成的订单到当月
                                if(Convert.ToDateTime(array[i]["send_date"].ToString())< Convert.ToDateTime(MonthFirst))
                                {
                                    send_date = DateTime.Now.ToString();
                                }
                                else
                                {
                                    send_date = array[i]["send_date"].ToString();
                                }
                                if (checkstatus)
                                {
                                    //单据类型
                                    ...
                                    if (isSH)
                                    {
                                        ...
                                        if (QtRow.Length == 0)
                                        {
                                            ...
                                            //单据业务日期
                                            bill_view.Model.SetValue("FDate", send_date);
                                            //领料部门
                                                ...
                                                if (rowWL.Length == 0)
                                                {
                                                    ...
                                                    if (rowWL2.Length > 0)
                                                    {
                                                        foreach (DataRow dr in rowWL2)
                                                        {
                                                            ...
                                                            FISBATCHMANAGE = dr["FNUMBER"].ToString();
                                                        }
                                                    }
                                                    else //物料不存在
                                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        if (array[i].ToString().Contains("receiver_country"))
                                        {
                                            ...
                                            for (int j = 0; j < details.Count; j++)
                                            {
                                                ...
                                                else if (array[i]["receiver_country"].ToString() == "US" || array[i]["receiver_country"].ToString() == "CA" || array[i]["receiver_country"].ToString() == "AU" || array[i]["receiver_country"].ToString() == "UM")
                                                {
                                                    //含税单价（填写的单价）
                                                    FTaxPrice = Convert.ToDecimal(details[j]["up"].ToString());
                                                    if (details[j].ToString().Contains("real_amtn"))
                                                ...
                                                else if (array[i]["receiver_country"].ToString() == "JP" || array[i]["receiver_country"].ToString() == "SG")
                                                {
                                                    ...
                                                    if (details[j].ToString().Contains("real_amtn"))
                                                    {
                                                        ...
                                                        if (promotion_discount > 0 && prdt_tax_fee > 0)
                                                        {
                                                            ...
                                                            {
                                                                //目前日本站未获取到折扣税
                                                                if (array[i]["receiver_country"].ToString() != "JP")
                                                                {
                                                                    decimal promotion_rebates_tax = Math.Abs(Convert.ToDecimal(details[j]["promotion_rebates_tax"].ToString()));
                                                ...
                                                else
                                                {
                                                    ...
                                                    //要货日期DateTime.Now.AddDays(10
                                                    bill_view.Model.SetValue("FDeliveryDate", send_date, j);
                                                    //计划交货日期
                                                    bill_view.Model.SetValue("FMinPlanDeliveryDate", send_date, j);
                                                    //结算组织
                                                    bill_view.Model.SetItemValueByID("FSettleOrgIds", FSaleOrgId, j);
                                                }
                                            }
